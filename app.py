...
# 2. Initial Data Quality Assessment
    ...
    if st.session_state.raw_df is None:
        ...
        if st.button("Assess Data Quality"):
            ...
            st.markdown(f"### 3.1 Missing Value Analysis:")
            ...
            st.markdown(f"### Missing Data Pattern Across Features and Companies:")
            fig, ax = plt.subplots(figsize=(12, 6))
            ...
            st.markdown(f"### 3.2 Distributional Summary (Key Percentiles for Outlier Detection):")
            desc = st.session_state.raw_df.describe(percentiles=[.01, .05, .25, .50, .75, .95, .99]).T[['count', 'mean', 'std', '1%', '99%']]
            st.dataframe(desc.round(2))

            st.markdown(f"### 3.3 Common Financial Data Anomalies:")
            ...
            st.markdown(f"### 3.4 Sector Distribution:")
            if 'sector' in st.session_state.raw_df.columns:
                st.dataframe(st.session_state.raw_df['sector'].value_counts())
            else:
                st.markdown(f"Sector column not found for distribution analysis.")
